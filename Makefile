...
# in object file. So here we say the module file(s) which will create .ko(s) is "shiki.o" and that other objects which
# must be linked (shiki-objs variable)
obj-m += shiki.o
shiki-objs := $(OBJS)
ccflags-y += -std=gnu99 -fgnu89-inline -Wno-declaration-after-statement
ccflags-y += -I$(src)/compat/toolkit/include

ifndef SK_VERSION_POSTFIX
SK_VERSION_POSTFIX := $(shell git rev-parse --is-inside-work-tree 1>/dev/null 2>/dev/null && echo -n "git-" && git log -1 --pretty='%h' 2>/dev/null || date '+at-%Y_%m_%d-%H_%M_%S')
endif
ccflags-y += -DSK_VERSION_POSTFIX="\"$(SK_VERSION_POSTFIX)\""
...
ifdef SK_MODULE_TARGET
ccflags-dev = -g -fno-inline -DDEBUG
ccflags-test = -O3
ccflags-prod = -O3
ccflags-y += -DSK_MODULE_TARGET_VER=${SK_MODULE_TARGET_VER} # this is assumed to be defined when target is specified

$(info SK-TARGET SPECIFIED AS ${SK_MODULE_TARGET} v${SK_MODULE_TARGET_VER})
...
ccflags-y += ${ccflags-${SK_MODULE_TARGET}}
else
# during the first read of the makefile we don't get the SK_MODULE_TARGET - if for some reason we didn't get it during
...
ccflags-y = --bogus-flag-which-should-not-be-called-NO_SK_MODULE_TARGER_SPECIFIED
endif
...
dev-v6: # kernel running in v6.2+ OS, all symbols included, debug messages included
	$(MAKE) -C $(LINUX_SRC) M=$(PWD) SK_MODULE_TARGET="dev" SK_MODULE_TARGET_VER="6" modules
test-v6: # kernel running in v6.2+ OS, fully stripped with only warning & above (no debugs or info)
	$(MAKE) -C $(LINUX_SRC) M=$(PWD) SK_MODULE_TARGET="test" SK_MODULE_TARGET_VER="6" modules
prod-v6: # kernel running in v6.2+ OS, fully stripped with no debug messages
	$(MAKE) -C $(LINUX_SRC) M=$(PWD) SK_MODULE_TARGET="prod" SK_MODULE_TARGET_VER="6" modules

# All v7 targets
dev-v7: # kernel running in v6.2+ OS, all symbols included, debug messages included
	$(MAKE) -C $(LINUX_SRC) M=$(PWD) SK_MODULE_TARGET="dev" SK_MODULE_TARGET_VER="7" modules
test-v7: # kernel running in v6.2+ OS, fully stripped with only warning & above (no debugs or info)
	$(MAKE) -C $(LINUX_SRC) M=$(PWD) SK_MODULE_TARGET="test" SK_MODULE_TARGET_VER="7" modules
prod-v7: # kernel running in v6.2+ OS, fully stripped with no debug messages
	$(MAKE) -C $(LINUX_SRC) M=$(PWD) SK_MODULE_TARGET="prod" SK_MODULE_TARGET_VER="7" modules
