...
#define SK_VERSION_POSTFIX "(NULL)"
#endif
#define SK_VERSION_MAJOR 0
#define SK_VERSION_MINOR 6
#define STRINGIFY(x) #x
#define VERSIONIFY(major,minor,postfix) "v" STRINGIFY(major) "." STRINGIFY(minor) "-" postfix
#define SK_VERSION_STR VERSIONIFY(SK_VERSION_MAJOR, SK_VERSION_MINOR, SK_VERSION_POSTFIX)
 ...
 */
void noinline __noreturn sk_crash(void) {
...
static int __init init_(void)
{
    ...
    pr_loc_dbg("================================================================================================");
    pr_loc_inf("shiki %s loading...", SK_VERSION_STR);
    ...
    pr_loc_inf("shiki %s loaded successfully (stealth=%d)", SK_VERSION_STR, STEALTH_MODE);
    return 0;

    error_out:
        pr_loc_crt("shiki %s cannot be loaded, initializer error=%d", SK_VERSION_STR, out);
...
{
    pr_loc_inf("shiki %s unloading...", SK_VERSION_STR);
    ...
    pr_loc_inf("shiki %s is dead", SK_VERSION_STR);
    pr_loc_dbg("================================================================================================");
}
module_exit(cleanup_);

MODULE_AUTHOR("TTG");
MODULE_VERSION(SK_VERSION_STR);
#endif
