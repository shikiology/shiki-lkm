...
static int sd_probe_shim(struct device *dev)
{
    ...
    int out = notifier_to_errno(blocking_notifier_call_chain(&sk_scsi_notify_list, SCSI_EVT_DEV_PROBING, sdp));
    ...
    blocking_notifier_call_chain(&sk_scsi_notify_list, evt, sdp);
...
{
    notifier_sub(nb);
    return blocking_notifier_chain_register(&sk_scsi_notify_list, nb);
}

int unsubscribe_scsi_disk_events(struct notifier_block *nb)
{
    notifier_unsub(nb);
    return blocking_notifier_chain_unregister(&sk_scsi_notify_list, nb);
}
