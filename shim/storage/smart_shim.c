...
static int populate_ata_id(const u8 *req_header, void __user *buff_ptr, const char* const disk_name)
{
    ...
    kzalloc_or_exit_int(kbuf, HDIO_DRIVE_CMD_HDR_OFFSET + sizeof(struct sk_hd_driveid));
    struct sk_hd_driveid *did = (void *)(kbuf + HDIO_DRIVE_CMD_HDR_OFFSET); //did=drive ID
    ...
    if (unlikely(copy_to_user(buff_ptr, kbuf, HDIO_DRIVE_CMD_HDR_OFFSET + sizeof(struct sk_hd_driveid)) != 0)) {
